...
class MaskRCNN:
    ...
    def detect_objects_mask(self, bgr_frame):
        ...
        for i in range(detection_count):# loop from 0 to detection_count
            ...
            if score < self.detection_threshold:# if the confidence score is less than 0.7 we will not going to display that specific object
                continue
            ...
            mask = cv2.resize(mask, (roi_width, roi_height))# resize the size of mask with the size of the object
            # if the confidence is greater than 0.3 we will mask it with a white pixel which value is 255 (object we want to detect)
            ...
            # cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE with this two commands we are going to extract the boundaries
